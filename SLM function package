...
SLM = function(formula, data, locations = NULL, WMatrix = NULL, k = NULL, alpha = 1, intercept = TRUE, Durbin = FALSE, 
               complete = FALSE, tol = 10^(-5), omit = FALSE, method = c("IDW", "Gaussian")){
  d = data
  ...
  ret = list(LR = LR, coef = coef.hat, locations = locations, fitted.values = fitted.values, residuals = residuals, Weight.Matrix = W)
  ret
}

Smod = SLM(beta0 ~ el2, data = X, locations = X[,4:3], k = 10, alpha = 1.5, method = "IDW", Durbin = TRUE, complete = TRUE)
plot(newmap, xlim = c(109.4,154.6), ylim = c(-43.65,-10.58), main = "Predicted Intercept")
scatter2D(Smod$locations[,1], Smod$locations[,2], colvar = Smod$fitted.values, cex = 0.2, add = TRUE)

SLM.predict(object, newdata, newlocations = NULL, new.Weight.Matrix = NULL){
  if(is.null(new.Weight.Matrix) & !is.ull(newlocations)){
    k = object$k
    alpha = object$alpha
    method = object$method
    tol = object$tol
    locations = object$locations
    new.Weight.Matrix = Sep.Weight.Matrix(Oldlocations = locations, Newlocations = newlocations, k = k, alpha = alpha, method = method, tol = tol)
    beta = object$coef[[2]]
    new.fitted.values = newdata
  }
}
